 ...
 *
 */
define(['lodash', 'jquery', 'core/promise', 'taoQtiItem/qtiItem/core/Loader', 'taoQtiItem/qtiCreator/model/Container', 'taoQtiItem/qtiCreator/model/Item', 'taoQtiItem/qtiCreator/model/helper/event', 'taoQtiItem/qtiCreator/model/qtiClasses', 'taoQtiItem/qtiCreator/helper/commonRenderer', 'taoQtiItem/qtiCreator/helper/xmlRenderer', 'taoQtiItem/qtiItem/helper/simpleParser', 'taoQtiItem/qtiCreator/helper/creatorRenderer', 'taoQtiItem/qtiCreator/helper/xincludeRenderer', 'taoQtiItem/qtiCreator/editor/gridEditor/content',
'nmcGapMatchInteraction/creator/tao/htmlEditor'], function (_, $, Promise, Loader, Container, Item, event, allQtiClasses, commonRenderer, xmlRenderer, simpleParser, creatorRenderer, xincludeRenderer, content, htmlEditor) {
    "use strict";
    ...
    function parser($container) {
        ...
        var newElems = {};
        Object.keys(data.body.elements).forEach(function(key){
            var elem = data.body.elements[key];
            if(elem.qtiClass !== "math"){
                newElems[key] = elem;
                return;
            }
            data.body.body = data.body.body.replace('{{'+key+'}}', '<math>'+elem.mathML + '</math>');
        })
        data.body.elements = newElems;
        if (data.body) {
            return data.body;
        } else {
    ...
    function create($container, options) {
        ...
        loader.loadRequiredClasses(data, function () {
            ...
            var item,
                containerEditors,
                renderer,
                qtiClasses = ['img', 'object',  'include', 'printedVariable', '_container', '_tooltip'];
            ...
            renderer.load(function () {
                ...
                $container.off('.' + _ns).on(event.getList(_ns + event.getNs() + event.getNsModel()).join(' '), _.throttle(function () {
    ...
    function cleanup($container) {
        ...
        return new Promise(function (resolve) {
            if (container) {
                ...
                commonRenderer.load(['img', 'object', 'include', '_container', 'printedVariable', '_tooltip'], function () {
    ...
    function destroy($editableContainer) {
        return htmlEditor.destroyEditor($editableContainer).then(function () {
