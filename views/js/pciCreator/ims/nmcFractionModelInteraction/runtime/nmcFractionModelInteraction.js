...
define([
    ...
    'nmcFractionModelInteraction/runtime/libs/pieChart',
    'css!nmcFractionModelInteraction/runtime/css/nmcFractionModelInteraction'
    ...
    function createCanvas($container, config){
        ...
        var canvas;
        var radix = 10,
            type = parseInt(config.type, radix),
            radius = parseInt(config.radius, radix),
            width = parseInt(config.width, radix),
            height = parseInt(config.height, radix),
            padding = parseInt(config.padding, radix),
            outlineThickness = parseInt(config.outlineThickness, radix);

        if (type === 0) {
            var canvasSize = 2 * (radius + padding + outlineThickness);
            canvas = scaleRaphael($('.shape-container', $container)[0], canvasSize, canvasSize);
        } else {
            var canvasWidth = width + 2*(padding + outlineThickness);
            var canvasHeight = height + 2*(padding + outlineThickness);
            canvas = scaleRaphael($('.shape-container', $container)[0], canvasWidth, canvasHeight);
        }
        
        return canvas;
    }

    var nmcFractionModelInteraction = {

        /**
         * return {Boolean} - Are we in a TAO QTI Creator context?
         */
        inQtiCreator: function isInCreator() {
            if (_.isUndefined(this._inQtiCreator) && this.dom) {
                this._inQtiCreator = $(this.dom).find('.nmcFractionModelInteraction').hasClass('tao-qti-creator-context');
            }
        ...
        getResponse : function(){
            ...
            return {
                record : [
                    {
                        name : 'response',
                        base : {string :  numerator + '/' + denominator}
                    }
                ]
            };
        },
        ...
        getTypeIdentifier : function(){
            return 'nmcFractionModelInteraction';
        },
        /**
         * Render the PCI :
         * @param {String} id
        ...
        initialize : function(id, dom, config){
            ...
            this.id = id;
            this.dom = dom;
            this.config = config = _.assign(config, { changeSelected: this.inQtiCreator() }) || {};
            ...
            var partitionsResetValue = _this.config.partitionInit;
            var origSelectedPartitions = JSON.parse(config.selectedPartitions) || [];
            ...
            this.setFractionModel = function setFractionModel(num, den){
                numerator = parseInt(num, radix);
                denominator = parseInt(den, radix);
            };
            ...
            $container.on('click.fraction', 'button.more', function(){
                ...
                if(denominator < _this.config.partitionMax){
                    ...
                    var selected = false;
                    if(!_this.config.changeSelected && newState.length < origSelectedPartitions.length){
                        selected = origSelectedPartitions[newState.length];
                    }
                ...
                if ($container.find('.tao-qti-creator-context').length !== 0) {
                    _this.setFractionModel([], Math.max(partitionsResetValue, _this.config.partitionMin));
                    _this.setState([]);
            ...
            this.on('configchange', function(config){
                _this.config = _.assign(config, { changeSelected: _this.inQtiCreator() });
                _this.setFractionModel(_this.config.selectedPartitionsInit, _this.config.partitionInit);
                _this.setState(_this.config.selectedPartitions);
            ...
            if (!$container.find('.tao-qti-creator-context').length && _this.config.partitionMin === _this.config.partitionMax) {
                $container.find('button.fewer').hide();
        ...
        setResponse : function(response){
            var radix = 10, i, parts, state = [], numerator, denominator, value = '0';
            if(response &&
                ...
                value = response.record[0].base.string;

                parts = value.split('/');
                numerator = parseInt(parts[0], radix);
                denominator = parseInt(parts[1] || 0, radix);
                for(i = 0; i< denominator; i++){
    ...
    qtiCustomInteractionContext.register(nmcFractionModelInteraction);
});
