...
center&&(wrapper.style.position="absolute",wrapper.style.left=parseInt((w-newWidth)/2)+"px",wrapper.style.top=parseInt((h-newHeight)/2)+"px")},paper.scaleAll=function(amount){paper.changeSize(width*amount,height*amount)},paper.changeSize(width,height),paper.w=width,paper.h=height,paper}}),define("nmcFractionModelInteraction/runtime/libs/pieChart",["taoQtiItem/portableLib/raphael","taoQtiItem/portableLib/lodash"],function(Raphael,_){"use strict";function getTotalSelected(selectedPartitions){return _.compact(_.values(selectedPartitions)).length}Raphael.fn.pieChart=function(selectedPartitions,config,$container){
function sector(cx,cy,r,startAngle,endAngle,params){var x1=cx+r*Math.cos(-startAngle*rad),x2=cx+r*Math.cos(-endAngle*rad),y1=cy+r*Math.sin(-startAngle*rad),y2=cy+r*Math.sin(-endAngle*rad);return paper.path(["M",cx,cy,"L",x1,y1,"A",r,r,0,+(endAngle-startAngle>180),0,x2,y2,"z"]).attr(params)}function bar(cx,cy,w,l,params){return paper.rect(cx,cy,w,l).attr(params)}function getClickCallback(j){return function(){String(this.attrs.fill)===config.partitionColor?(this.attr(getStyle(!0)),selectedPartitions[j]=!0,$container.trigger("select_slice.pieChart",[selectedPartitions,getTotalSelected(selectedPartitions)])):(this.attr(getStyle(!1)),selectedPartitions[j]=!1,
$container.trigger("unselect_slice.pieChart",[selectedPartitions,getTotalSelected(selectedPartitions)]))}}var paper=this,chart=this.set(),type=0===Number(config.type)?"circle":"rectangle",subtype=0===Number(config.subtype)?"rows":"columns",padding=config.padding||2,r=parseInt(config.radius,10),width=parseInt(config.width,10),height=parseInt(config.height,10),cx="circle"===type?r+padding:padding,cy="circle"===type?r+padding:padding,rad=Math.PI/180,getStyle=function(selected){return{fill:selected?config.selectedPartitionsColor:config.partitionColor,cursor:"pointer",stroke:config.outlineColor,"stroke-width":config.outlineThickness}
},setHoverStyle=function(raphaelObject){raphaelObject.hover(function(){this.animate({"fill-opacity":.7})},function(){this.stop().attr({"fill-opacity":1})})},origSelectedPartitions=JSON.parse(config.selectedPartitions);selectedPartitions=_.clone(selectedPartitions);var angle=0,angleplus=360/selectedPartitions.length,yPos=cy,yHeight=height/selectedPartitions.length,xPos=cx,xWidth=width/selectedPartitions.length;if(selectedPartitions.length>1)for(var i=0;i<selectedPartitions.length;i++)!function(j,type,subtype){var p,selected=selectedPartitions[j];selectedPartitions[j]=selected,"circle"===type?(p=sector(cx,cy,r,angle,angle+angleplus,getStyle(selected)),
angle+=angleplus):("rows"===subtype&&(p=bar(cx,yPos,width,yHeight,getStyle(selected)),yPos+=yHeight),"columns"===subtype&&(p=bar(xPos,cy,xWidth,height,getStyle(selected)),xPos+=xWidth)),setHoverStyle(p),chart.push(p),(config.changeSelected||j>=origSelectedPartitions.length||!origSelectedPartitions[j])&&p.click(getClickCallback(j))}(i,type,subtype);else"rectangle"===type?function(){var selected=selectedPartitions[0],rect=paper.rect(cx,cy,width,height).attr(getStyle(selected));setHoverStyle(rect),chart.push(rect),(config.changeSelected||0>=origSelectedPartitions.length||!origSelectedPartitions[0])&&rect.click(getClickCallback(0))}():function(){
var selected=selectedPartitions[0],c=paper.circle(cx,cy,r).attr(getStyle(selected));setHoverStyle(c),chart.push(c),(config.changeSelected||0>=origSelectedPartitions.length||!origSelectedPartitions[0])&&c.click(getClickCallback(0))}();$container.trigger("drawn.pieChart")}}),define("css!nmcFractionModelInteraction/runtime/css/nmcFractionModelInteraction",[],function(){}),
define("nmcFractionModelInteraction/runtime/nmcFractionModelInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/scale.raphael","nmcFractionModelInteraction/runtime/libs/pieChart","css!nmcFractionModelInteraction/runtime/css/nmcFractionModelInteraction"],function($,qtiCustomInteractionContext,event,_,scaleRaphael){"use strict";function createCanvas($container,config){config=_.defaults(config,_defaults)
;var canvas,type=parseInt(config.type,10),radius=parseInt(config.radius,10),width=parseInt(config.width,10),height=parseInt(config.height,10),padding=parseInt(config.padding,10),outlineThickness=parseInt(config.outlineThickness,10);if(0===type){var canvasSize=2*(radius+padding+outlineThickness);canvas=scaleRaphael($(".shape-container",$container)[0],canvasSize,canvasSize)}else{var canvasWidth=width+2*(padding+outlineThickness),canvasHeight=height+2*(padding+outlineThickness);canvas=scaleRaphael($(".shape-container",$container)[0],canvasWidth,canvasHeight)}return canvas}var _defaults={type:0,subtype:0,radius:100,width:200,height:200,padding:10,
outlineThickness:1},nmcFractionModelInteraction={inQtiCreator:function(){return _.isUndefined(this._inQtiCreator)&&this.dom&&(this._inQtiCreator=$(this.dom).find(".nmcFractionModelInteraction").hasClass("tao-qti-creator-context")),this._inQtiCreator},typeIdentifier:"graphLineAndPointInteraction",getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getSelectedPartitions())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},
getResponse:function(){var selection=_.values(this.getSelectedPartitions());return{record:[{name:"response",base:{string:_.filter(selection).length+"/"+selection.length}}]}},destroy:function(){$(this.dom).off().empty()},getTypeIdentifier:function(){return"nmcFractionModelInteraction"},initialize:function(id,dom,config){event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config=_.assign(config,{changeSelected:this.inQtiCreator()})||{};var numerator,denominator,selectedPartitions,_this=this,$container=$(dom),partitionsResetValue=_this.config.partitionInit,origSelectedPartitions=JSON.parse(config.selectedPartitions)||[]
;this.setFractionModel=function(num,den){numerator=parseInt(num,10),denominator=parseInt(den,10)},this.getFractionModel=function(){return{numerator:numerator,denominator:denominator}},this.setState=function(stateData){var i;if(_.isString(stateData))""===stateData.trim()||"[]"===stateData.trim()?selectedPartitions=_.range(denominator).map(function(i){return i<numerator}):(selectedPartitions=JSON.parse(stateData),denominator=selectedPartitions.length);else{if(!_.isArray(stateData))throw"invalid format for selectedPartitions";if(stateData.length)selectedPartitions=stateData,denominator=selectedPartitions.length;else for(selectedPartitions=[],
i=0;i<denominator;i++)selectedPartitions.push(!1)}},this.getSelectedPartitions=function(){return _.clone(selectedPartitions)},this.setFractionModel(this.config.selectedPartitionsInit,this.config.partitionInit);var canvas=createCanvas($container,config);this.setState(config.selectedPartitions),canvas.pieChart(this.getSelectedPartitions(),this.config,$container),$container.on("click.fraction","button.more",function(){if(denominator<_this.config.partitionMax){denominator+=1;var newState=_this.getSelectedPartitions(),selected=!1;!_this.config.changeSelected&&newState.length<origSelectedPartitions.length&&(selected=origSelectedPartitions[newState.length]),
newState.push(selected),_this.setState(newState),$container.trigger("statechange.fraction")}}).on("click.fraction","button.fewer",function(){if(denominator>_this.config.partitionMin){denominator-=1;var newState=_this.getSelectedPartitions();newState.pop(),_this.setState(newState),$container.trigger("statechange.fraction")}}).on("click.fraction","button.reset",function(){0!==$container.find(".tao-qti-creator-context").length?(_this.setFractionModel([],Math.max(partitionsResetValue,_this.config.partitionMin)),_this.setState([]),$container.trigger("reset.fraction")):(_this.setFractionModel(_this.config.selectedPartitionsInit,_this.config.partitionInit),
_this.setState(_this.config.selectedPartitions),$container.trigger("reset.fraction"))}).on("statechange.fraction reset.fraction",function(event){canvas.clear(),canvas.pieChart(_this.getSelectedPartitions(),_this.config,$container),_this.trigger("changepartition",[_this.getSelectedPartitions()]),_this.trigger("responseChange",[_this.getResponse()])}).on("select_slice.pieChart unselect_slice.pieChart",function(e,selectedPartitions,totalSelected){numerator=totalSelected,_this.setState(selectedPartitions),_this.trigger("selectedpartition",[selectedPartitions]),_this.trigger("responseChange",[_this.getResponse()])}),this.on("configchange",function(config){
_this.config=_.assign(config,{changeSelected:_this.inQtiCreator()}),_this.setFractionModel(_this.config.selectedPartitionsInit,_this.config.partitionInit),_this.setState(_this.config.selectedPartitions),canvas.clear(),canvas=createCanvas($container,_this.config),canvas.pieChart(_this.getSelectedPartitions(),_this.config,$container)}),$container.find(".tao-qti-creator-context").length||_this.config.partitionMin!==_this.config.partitionMax||($container.find("button.fewer").hide(),$container.find("button.more").hide())},setResponse:function(response){var i,parts,numerator,denominator,state=[],value="0"
;if(response&&_.isArray(response.record)&&response.record[0]&&"response"===response.record[0].name&&response.record[0].base&&response.record[0].base.string){for(value=response.record[0].base.string,parts=value.split("/"),numerator=parseInt(parts[0],10),denominator=parseInt(parts[1]||0,10),i=0;i<denominator;i++)state.push(i<numerator);this.setState(state),$(this.dom).trigger("statechange.fraction")}},resetResponse:function(){this.setFractionModel(this.config.selectedPartitionsInit,this.config.partitionInit),this.setState(this.config.selectedPartitions),$(this.dom).trigger("statechange.fraction")},setSerializedState:function(state){
state&&_.isArray(state.selection)&&(this.setState(state.selection),$(this.dom).trigger("statechange.fraction"))},getSerializedState:function(){return{selection:_.values(this.getSelectedPartitions())}}};qtiCustomInteractionContext.register(nmcFractionModelInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))
}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\r\n\r\n.nmcFractionModelInteraction {width:100%;}\r\n\r\n.nmcFractionModelInteraction .likert-label {min-width:90px; color: #444;}\r\n.nmcFractionModelInteraction .likert-label-min {text-align: right;}\r\n.nmcFractionModelInteraction .likert-label-max {text-align: left;}\r\n\r\n.nmcFractionModelInteraction .shape-title {margin-top: 10px; margin-right : auto; margin-left : auto; width : 50%; text-align : center;}\r\n.nmcFractionModelInteraction .shape-panel {padding: 10px 10px 20px; background : #EFEFEF;}\r\n.nmcFractionModelInteraction .shape-container {margin:auto;}\r\n.nmcFractionModelInteraction .shape-controls {text-align: center;}\r\n.nmcFractionModelInteraction .shape-controls button {margin: 0 5px; padding: 0 20px;}\r\n"),
define(["nmcFractionModelInteraction/runtime/nmcFractionModelInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=nmcFractionModelInteraction.min.js.map