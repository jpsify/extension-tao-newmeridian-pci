...
},path="M"+startPosition.left+" "+startPosition.top+"L"+middlePosition.left+" "+middlePosition.top+"L"+endPosition.left+" "+endPosition.top;return canvas.path(path)}},quadratic:quadratic,exponential:exponential,logarithmic:logarithmic,cosine:cosine,tangent:tangent,vertical:{get:function(point){if(void 0!==point.x)return[point.x]},plot:function(canvas,equation,config){var multiplier;multiplier=!0===config.segment?-1:1;var x=equation[0]*config.unitSize.x+config.origin.left,yStart=multiplier*config.start_y*config.unitSize.y+config.origin.top,yEnd=multiplier*config.end_y*config.unitSize.y+config.origin.top,path="M"+x+" "+yStart+"L"+x+" "+yEnd
;return canvas.path(path)}}}}),define("nmcPci/portableLib/plotFactory",["taoQtiItem/portableLib/lodash","nmcPci/portableLib/graphFunction"],function(_,graphFunction){"use strict";function _applyStyle(path,config){path.attr({stroke:config.color?config.color:_defaults.color,"stroke-width":config.thickness?config.thickness:_defaults.thickness,opacity:config.opacity?config.opacity:_defaults.opacity})}function PlotFactory(grid,config0){function _translateCoordinate(point){return{x:(point.getX()-config.origin.left)/config.unitSize.x,y:-(point.getY()-config.origin.top)/config.unitSize.y}}function _plot(fnName,p1,p2,conf){
var equation,plot,point1=_translateCoordinate(p1),point2=_translateCoordinate(p2);conf=_.defaults(conf||{},config),conf.point1=point1,conf.point2=point2;try{if(equation=graphFunction[fnName].get(point1,point2),equation||(fnName=point1.x===point2.x?"vertical":"linear",equation=graphFunction[fnName].get(point1,point2)),equation){if(conf.segment&&(conf.start=Math.min(point1.x,point2.x),conf.end=Math.max(point1.x,point2.x),conf.start_y=point1.y,conf.end_y=point2.y),equation.type=fnName,conf.includeBorders&&!conf.segment){var shiftX=2*conf.thickness/conf.unitSize.x,shiftY=2*conf.thickness/conf.unitSize.y;conf.start=conf.start-shiftX,
conf.start_y=conf.start_y-shiftY,conf.end=conf.end+shiftX,conf.end_y=conf.end_y+shiftY}if(conf.borderX1=conf.start*conf.unitSize.x+conf.origin.left,conf.borderY1=conf.origin.top+conf.start_y*conf.unitSize.y,conf.borderX2=conf.end*conf.unitSize.x+conf.origin.left,conf.borderY2=conf.origin.top+conf.end_y*conf.unitSize.y,plot=graphFunction[fnName].plot(canvas,equation,conf),!conf.segment&&!conf.includeBorders){var x1=conf.start*conf.unitSize.x+conf.origin.left,y1=conf.origin.top+conf.start_y*conf.unitSize.y,x2=conf.end*conf.unitSize.x+conf.origin.left-x1,y2=conf.origin.top+conf.end_y*conf.unitSize.y-y1;plot.attr({"clip-rect":x1+" "+y1+" "+x2+" "+y2})}
return plot.equation=equation,_applyStyle(plot,conf),plot}}catch(e){console.log(e)}return!1}var _this=this,canvas=grid.getCanvas(),config=_.clone(config0||{});config=_.defaults(config,_defaults);var bounds=grid.getGridBounds();config.unitSize=grid.getUnits(),config.origin=grid.getOriginPosition(),config.start=bounds.x.start,config.end=bounds.x.end,config.start_y=bounds.y.start,config.end_y=bounds.y.end;var availableFunctions=["vertical","linear","absolute","cosine","tangent","exponential","logarithmic","quadratic"];_.each(availableFunctions,function(fnName){_this[PlotFactory.getPlotName(fnName)]=function(point1,point2,conf){
return _plot(fnName,point1,point2,conf)}})}var _defaults={start:-10,end:10,precision:.01,opacity:.8,color:"#bb1a2a",thickness:3,includeBorders:!1};return PlotFactory.getPlotName=function(mathFunctionName){return"plot"+mathFunctionName.charAt(0).toUpperCase()+mathFunctionName.substr(1)},PlotFactory}),define("css!nmcGraphFunctionInteraction/runtime/css/nmcGraphFunctionInteraction",[],function(){}),
define("nmcGraphFunctionInteraction/runtime/nmcGraphFunctionInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/scale.raphael","nmcPci/portableLib/gridFactory","nmcPci/portableLib/pointFactory","nmcPci/portableLib/plotFactory","css!nmcGraphFunctionInteraction/runtime/css/nmcGraphFunctionInteraction"],function($,qtiCustomInteractionContext,_,event,scaleRaphael,gridFactory,pointFactory,PlotFactory){"use strict";function extractPointsFromString(stringPoints){var points=[];return _.each(stringPoints.split(/,/),function(strPoint){
var pt=strPoint.trim().split(/\s+/);if(2!==pt.length)return!1;points.push({x:pt[0],y:pt[1]})}),points}var nmcGraphFunctionInteraction,buildGridConfig=function(rawConfig){var getBoolean=function(value,defaultValue){return void 0===value?defaultValue:!0===value||"true"===value},xWeight=parseInt(rawConfig.xWeight,10),xBorderWeight=parseInt(rawConfig.xBorderWeight,10),yWeight=parseInt(rawConfig.yWeight,10),yBorderWeight=parseInt(rawConfig.yBorderWeight,10),gridConfig={graphType:rawConfig.graphType,graphTitle:rawConfig.graphTitle,graphTitleRequired:!0,weight:parseInt(rawConfig.weight,10),width:parseInt(rawConfig.width,10),height:parseInt(rawConfig.height,10),
showOrigin:getBoolean(rawConfig.showOrigin,!0),showXAxisTitle:getBoolean(rawConfig.showXAxisTitle,!0),showYAxisTitle:getBoolean(rawConfig.showYAxisTitle,!0),showSubGrid:getBoolean(rawConfig.showSubGrid,!1),subGridColor:rawConfig.subGridColor,x:{start:parseInt(rawConfig.xStart,10),end:parseInt(rawConfig.xEnd,10),label:rawConfig.xLabel,title:rawConfig.xTitle,step:parseFloat(rawConfig.xStep,10),subStep:parseInt(rawConfig.xSubStep,10),digits:parseInt(rawConfig.xDigits,10),weight:xWeight>0?xWeight:xBorderWeight,allowOuter:getBoolean(rawConfig.xAllowOuter,!0),dash:!1},y:{start:-1*parseInt(rawConfig.yEnd,10),end:-1*parseInt(rawConfig.yStart,10),
label:rawConfig.yLabel,title:rawConfig.yTitle,step:parseFloat(rawConfig.yStep,10),subStep:parseInt(rawConfig.ySubStep,10),digits:parseInt(rawConfig.yDigits,10),weight:yWeight>0?yWeight:yBorderWeight,allowOuter:getBoolean(rawConfig.yAllowOuter,!0),dash:!1},plot:{color:rawConfig.plotColor,thickness:parseInt(rawConfig.plotThickness,10)},point:{color:rawConfig.pointColor,glow:getBoolean(rawConfig.pointGlow,!0),radius:parseInt(rawConfig.pointRadius,10)}};return gridConfig.x.step<.1&&(gridConfig.x.step=.1),gridConfig.y.step<.1&&(gridConfig.y.step=.1),gridConfig.x.subStep<1&&(gridConfig.x.subStep=1),gridConfig.y.subStep<1&&(gridConfig.y.subStep=1),
gridConfig.x.weight>0==!1&&(gridConfig.x.weight=3),gridConfig.y.weight>0==!1&&(gridConfig.y.weight=3),gridConfig},createCanvas=function($container,config){var paperSize=gridFactory.getPaperSize(config);return scaleRaphael($(".shape-container",$container)[0],paperSize.width,paperSize.height)};nmcGraphFunctionInteraction={typeIdentifier:"nmcGraphFunctionInteraction",getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){
return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var raw=this.getRawResponse();return raw?{record:[{name:"functionGraphType",base:{string:raw.mathFunction}},{name:"points",base:{string:raw.point1.x+" "+raw.point1.y+","+raw.point2.x+" "+raw.point2.y}}]}:{base:null}},destroy:function(){$(this.dom).off().empty()},getTypeIdentifier:function(){return"nmcGraphFunctionInteraction"},initialize:function(id,dom,config){function initGrid($container,gridConfig){return clearPlot(),clearPoint(),paper=createCanvas($container,gridConfig),
_.isObject(gridConfig.x)&&_.isObject(gridConfig.y)&&gridConfig.x.start<gridConfig.x.end&&gridConfig.y.start<gridConfig.y.end&&(grid=gridFactory(paper,gridConfig),grid.clickable(),grid.children.click(function(event){var oldPoint,bnds=event.target.getBoundingClientRect(),wfactor=paper.w/paper.width,fx=grid.getX()+Math.round((event.clientX-bnds.left)/bnds.width*grid.getWidth()*wfactor),fy=grid.getY()+Math.round((event.clientY-bnds.top)/bnds.height*grid.getHeight()*wfactor);areCoordsValid(fx,fy)&&(points.length<2?(addPoint(fx,fy),2===points.length&&plot()):(oldPoint=points.pop(),oldPoint.setCoord(fx,fy),oldPoint.render(),oldPoint.drag(),points.push(oldPoint),
plot()))}),plotFactory=new PlotFactory(grid,gridConfig.plot),$(paper.canvas).off("removed.point").on("removed.point",function(event,removedPoint){var pointToDelete=_.findIndex(points,{uid:removedPoint.uid});pointToDelete>-1&&(points.splice(pointToDelete,1),clearPlot())})),grid}function updateLabels(graphs){_.forEach(graphs,function(graph,key){$shapeControls.find("[name="+key+"]").html(graph.label)})}function showControl(graphs){$shapeControls.children("button").removeClass("available"),_.each(graphs,function(graph){$shapeControls.find("[name="+graph+"]").addClass("available")}),activateButton($shapeControls.children(".available:first"))}
function activateButton($button,preventDefaultPlot){if("string"==typeof $button&&($button=$shapeControls.find("[name="+$button+"]")),$button.length){var fnName=$button.val();mathFunction=PlotFactory.getPlotName(fnName),self.gridConfig.x.dash="plotExponential"===mathFunction,self.gridConfig.y.dash="plotLogarithmic"===mathFunction,initGrid($container,self.gridConfig),$button.removeClass("btn-info").addClass("btn-success"),$button.siblings("button").removeClass("btn-success").addClass("btn-info"),preventDefaultPlot||plotDefault()}}function clearPlot(){path&&path.remove()}function clearPoint(){_.each(points,function(point){point.remove()}),points=[]}
function plot(){var point1=points[0],point2=points[1];point1&&point2&&mathFunction&&plotFactory[mathFunction]&&(clearPlot(),path=plotFactory[mathFunction](point1,point2),self.trigger("responseChange",[self.getResponse()]))}function areCoordsValid(x,y,cartesian){var gridBBox,snappedPoint,xOnOuter,yOnOuter;return cartesian?(xOnOuter=x===self.gridConfig.x.start||x===self.gridConfig.x.end,yOnOuter=y===self.gridConfig.y.start||y===self.gridConfig.y.end):(gridBBox=grid.getBBox(),snappedPoint=grid.snap(x,y),xOnOuter=snappedPoint[0]===gridBBox.x||snappedPoint[0]===gridBBox.x2,yOnOuter=snappedPoint[1]===gridBBox.y||snappedPoint[1]===gridBBox.y2),
!(!1===self.gridConfig.x.allowOuter&&xOnOuter||!1===self.gridConfig.y.allowOuter&&yOnOuter)}function addPoint(fx,fy,cartesian){var gridBBox,newPoint,pointConfig,draggableArea,subStepSizes;return grid&&areCoordsValid(fx,fy,cartesian)&&(gridBBox=grid.getBBox(),subStepSizes=grid.getSubStepSizes(),draggableArea={xMin:self.gridConfig.x.allowOuter?gridBBox.x:gridBBox.x+subStepSizes.x,xMax:self.gridConfig.x.allowOuter?gridBBox.x2:gridBBox.x2-subStepSizes.x,yMin:self.gridConfig.y.allowOuter?gridBBox.y:gridBBox.y+subStepSizes.y,yMax:self.gridConfig.y.allowOuter?gridBBox.y2:gridBBox.y2-subStepSizes.y},pointConfig=_.defaults({x:fx,y:fy,xMin:draggableArea.xMin,
xMax:draggableArea.xMax,yMin:draggableArea.yMin,yMax:draggableArea.yMax,on:{dragStart:clearPlot,dragStop:plot}},self.gridConfig.point),newPoint=pointFactory(paper,grid,pointConfig),cartesian&&newPoint.setCartesianCoord(fx,fy,pointConfig),newPoint.render(),newPoint.drag(),points.push(newPoint)),newPoint}function getGraphOrigin(){var x0,y0,_y=self.gridConfig.y,_x=self.gridConfig.x;return y0=_y.start<0&&_y.end<0?_y.start:_y.start>=0&&_y.end>=0?_y.start:0,x0=_x.start<0&&_x.end<=0?_x.start:_x.start>=0&&_x.end>0?_x.start:0,{x:x0,y:y0}}function plotDefault(){clearPoint(),clearPlot();var origin=getGraphOrigin()
;"plotExponential"===mathFunction||"plotLogarithmic"===mathFunction?(addPoint(origin.x+1,origin.y+1,!0),addPoint(origin.x+2,origin.y+2,!0)):mathFunction&&(addPoint(origin.x+0,origin.y+0,!0),addPoint(origin.x+1,origin.y+1,!0)),mathFunction&&plot()}event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{};var paper,grid,plotFactory,path,mathFunction,$container=$(dom),graphs=$.parseJSON(config.graphs),mathFunctions=_.pairs(graphs).filter(function(graph){return graph[1].checked}).map(function(graph){return graph[0]}),$shapeControls=$container.find(".shape-controls"),self=this,points=[];this.getRawResponse=function(){
var point1=points[0],point2=points[1];if(point1&&point2&&mathFunction)return{point1:point1.getCartesianCoord(1),point2:point2.getCartesianCoord(1),mathFunction:mathFunction.replace("plot","").toLowerCase()}},this.setRawResponse=function(mathFn,point1,point2){clearPoint(),clearPlot(),mathFunction=mathFn||mathFunctions[0],activateButton(mathFunction,!0),point1&&point1.x&&point1.y&&addPoint(point1.x,point1.y,!0),point2&&point2.x&&point2.y&&addPoint(point2.x,point2.y,!0),plot()},this.gridConfig=buildGridConfig(this.config),initGrid($container,this.gridConfig),updateLabels($.parseJSON(this.config.graphs)),showControl(mathFunctions),plotDefault(),
$shapeControls.on("click","button",function(){activateButton($(this))}),self.on("configChange",function(newRawConfig){var state=self.getRawResponse();self.config=newRawConfig,self.gridConfig=buildGridConfig(newRawConfig),initGrid($container,self.gridConfig),self.setRawResponse(state)}),self.on("gridChange",function(newRawConfig){self.config=newRawConfig,self.gridConfig=buildGridConfig(newRawConfig),initGrid($container,self.gridConfig),plotDefault()}),self.on("functionsChange",function(graphs){mathFunction=null,updateLabels(graphs),mathFunctions=_.filter(_.pairs(graphs),function(graph){return graph[1].checked}).map(function(graph){return graph[0]}),
showControl(mathFunctions),plotDefault()})},setResponse:function(response){var points;response&&_.isArray(response.record)&&response.record[0]&&response.record[1]&&"functionGraphType"===response.record[0].name&&response.record[0].base&&response.record[0].base.string&&"points"===response.record[1].name&&response.record[1].base&&response.record[1].base.string&&(points=extractPointsFromString(response.record[1].base.string),this.setRawResponse(response.record[0].base.string,points.shift(),points.shift()))},resetResponse:function(){this.setRawResponse()},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){
return this.getResponse()}},qtiCustomInteractionContext.register(nmcGraphFunctionInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))
}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\r\n\r\n.nmcGraphFunctionInteraction {\r\n    width:100%;\r\n    padding:30px 30px 50px;\r\n    margin:20px 0;\r\n    padding:20px;\r\n    border : 6px solid #f5f3f2;\r\n}\r\n\r\n.nmcGraphFunctionInteraction .prompt {margin-bottom:20px;}\r\n.nmcGraphFunctionInteraction .shape-panel {position:relative; display: inline-block; width: 100%;}\r\n.nmcGraphFunctionInteraction .shape-panel .shape-container {float:left; margin-left:30px;}\r\n.nmcGraphFunctionInteraction .shape-panel .shape-controls {float:left; margin-bottom:20px;}\r\n.nmcGraphFunctionInteraction .shape-panel .shape-controls button {width:150px; margin:3px 0; display:none;}\r\n.nmcGraphFunctionInteraction .shape-panel .shape-controls button.available {display:block;}"),
define(["nmcGraphFunctionInteraction/runtime/nmcGraphFunctionInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=nmcGraphFunctionInteraction.min.js.map