...
center&&(wrapper.style.position="absolute",wrapper.style.left=parseInt((w-newWidth)/2)+"px",wrapper.style.top=parseInt((h-newHeight)/2)+"px")},paper.scaleAll=function(amount){paper.changeSize(width*amount,height*amount)},paper.changeSize(width,height),paper.w=width,paper.h=height,paper}}),define("nmcPci/portableLib/gridFactory",["taoQtiItem/portableLib/lodash"],function(_){"use strict";function gridFactory(paper,rawOptions){function _drawGraphTitle(){var x=_padding.left+_width/2,y=options.padding,style={font:"bold "+options.graphTitleSize+"pt Verdana"};options.graphTitle&&!0===options.graphTitleRequired&&_drawTitle(options.graphTitle,style,x,y)}
function _drawAxis(){function drawXaxis(top,config){config=config||{};var textTop,text,i,digits=_x.digits||1,line=_drawLine([0,top],[_width,top],config.style).attr({"stroke-width":2}),readabilityOffset=config.multiQuadrant?5:0,position=readabilityOffset;if(_x.arrows&&(_x.start<0&&line.attr({"arrow-start":arrowheadsShape}),_x.end>0&&line.attr({"arrow-end":arrowheadsShape})),_x.dash&&_drawLine([0,top],[_width,top],config.style).attr({"stroke-width":6,"stroke-dasharray":"- ",stroke:"grey"}),options.showXAxisTitle)for(textTop=config.unitsOnTop?top+_padding.top-options.labelSize:top+_padding.top+options.labelSize,position+=_x.unit*_x.firstStep,
i=_x.start+_x.firstStep;Number(i.toFixed(digits))<_x.end;i+=_x.step){if(0!==Number(i.toFixed(digits))){var label=_x.digits?i.toFixed(_x.digits):1*i.toFixed(digits);text=paper.text(_padding.left+position,textTop,label).attr(labelStyle);var box=text.getBBox();paper.rect(box.x,box.y,box.width,box.height).attr({fill:"#ffffff",stroke:"none"}),text.toFront(),_addCssClass(text,"scene scene-text")}position+=_x.unit*_x.step}return line}function drawYaxis(left,config){config=config||{};var textLeft,text,i,digits=_y.digits||1,line=_drawLine([left,_height],[left,0],config.style).attr({"stroke-width":2
}),readabilityOffset=config.multiQuadrant?-5:0,position=readabilityOffset;if(_y.dash&&_drawLine([left,_height],[left,0],config.style).attr({"stroke-width":6,"stroke-dasharray":"- ",stroke:"grey"}),_y.arrows&&(_y.start<0&&line.attr({"arrow-end":arrowheadsShape}),_y.end>0&&line.attr({"arrow-start":arrowheadsShape})),options.showYAxisTitle||options.showOrigin)for(textLeft=config.unitsOnRight?left+_padding.left+options.labelSize/2+2:left+_padding.left-options.labelSize-2,position+=_y.unit*_y.firstStep,i=_y.start+_y.firstStep;Number(i.toFixed(digits))<=_y.end;i+=_y.step){var isOrigin=0===Number(i.toFixed(digits)),isEnd=Number(i.toFixed(digits))===_y.end
;if(options.showYAxisTitle&&!isEnd&&!isOrigin||isOrigin&&options.showOrigin){var offset=isOrigin?options.labelSize:0,label=_y.digits?(-i).toFixed(_y.digits):-i.toFixed(digits);text=paper.text(textLeft+offset/2,_padding.top+position+offset,label).attr(_.merge({},labelStyle,{"text-anchor":"end"}));var box=text.getBBox();paper.rect(box.x,box.y,box.width,box.height).attr({fill:"#ffffff",stroke:"none"}),text.toFront(),_addCssClass(text,"scene scene-text")}position+=_y.unit*_y.step}return line}var xStyle={stroke:_x.color,"stroke-width":_x.weight},yStyle={stroke:_y.color,"stroke-width":_y.weight},axisTitleStyle={font:options.axisTitleSize+"pt Verdana Regular"
},labelStyle={font:options.scaleSize+"px Verdana Regular"},axisLabelStyle={font:options.labelSize+"px Verdana","font-style":"italic"};_y.start<0&&_y.end<=0?drawXaxis(_height,{style:xStyle}):_y.start>=0&&_y.end>0?drawXaxis(0,{style:xStyle,unitsOnTop:!0}):drawXaxis(Math.abs(_y.start)*_y.unit,{style:xStyle,multiQuadrant:!1}),_x.start<0&&_x.end<=0?drawYaxis(_width,{style:yStyle,unitsOnRight:!0}):_x.start>=0&&_x.end>0?drawYaxis(0,{style:yStyle}):drawYaxis(Math.abs(_x.start)*_x.unit,{style:yStyle,multiQuadrant:!1}),$("#raphael-marker-startblock55",paper.canvas).attr("id","raphael-marker-startblock55_grid"),
$("#raphael-marker-endblock55",paper.canvas).attr("id","raphael-marker-endblock55_grid"),$('path[marker-start="url(#raphael-marker-startblock55)"]',paper.canvas).attr("marker-start","url(#raphael-marker-startblock55_grid)"),$('path[marker-end="url(#raphael-marker-endblock55)"]',paper.canvas).attr("marker-end","url(#raphael-marker-endblock55_grid)"),_x.label&&_drawTitle(_x.label,axisLabelStyle,_padding.left+_labelCoords.abs.x,_padding.top+_labelCoords.abs.y,_labelCoords.abs.angle),_y.label&&_drawTitle(_y.label,axisLabelStyle,_padding.left+_labelCoords.ord.x,_padding.top+_labelCoords.ord.y,_labelCoords.ord.angle),
_x.title&&_drawTitle(_x.title,axisTitleStyle,_padding.left+_axisTitleCoords.abs.x,_padding.top+_axisTitleCoords.abs.y,_axisTitleCoords.abs.angle),_y.title&&_drawTitle(_y.title,axisTitleStyle,_padding.left+_axisTitleCoords.ord.x,_padding.top+_axisTitleCoords.ord.y,_axisTitleCoords.ord.angle)}function _drawGrid(){var x,y,subStep,style={stroke:_color,"stroke-width":_weight},subStyle={stroke:options.subGridColor,"stroke-width":1},borderStyle={stroke:_color,"stroke-width":options.borderWidth};if(_y.showGrid)for(y=_y.firstStep*_y.unit;y<=_height;y+=_y.step*_y.unit)if(_drawLine([0,y],[_width,y],style),options.showSubGrid){subStep=_y.step*_y.unit/_y.subStep
;for(var subY=y-subStep,sub=_y.subStep-1;sub>0&&subY>0;sub--)_drawLine([0,subY],[_width,subY],subStyle),subY-=subStep}if(Math.abs(_y.end)%_y.step){if(options.showSubGrid)for(var subY=y-_y.step*_y.unit+subStep,sub=_y.subStep-1;sub>0&&subY<_height;sub--)_drawLine([0,subY],[_width,subY],subStyle),subY+=subStep;_drawLine([0,_height],[_width,_height],style)}if(_x.showGrid){for(_drawLine([0,0],[0,_height],style),x=_x.firstStep*_x.unit;x<=_width;x+=_x.step*_x.unit)if(_drawLine([x,0],[x,_height],style),options.showSubGrid){subStep=_x.step*_x.unit/_x.subStep;for(var subX=x-subStep,sub=_x.subStep-1;sub>0&&subX>0;sub--)_drawLine([subX,0],[subX,_height],subStyle),
subX-=subStep}}else _drawLine([_width,0],[_width,_height],style);if(Math.abs(_x.end)%_x.step){if(options.showSubGrid)for(var subX=x-_x.step*_x.unit+subStep,sub=_x.subStep-1;sub>0&&subX<_width;sub--)_drawLine([subX,0],[subX,_height],subStyle),subX+=subStep;_drawLine([_width,0],[_width,_height],style)}var borderPadding=options.borderWidth/2,maxX=_width,maxY=_height;_drawLine([0-borderPadding,0],[maxX+borderPadding,0],borderStyle),_drawLine([maxX,0],[maxX,maxY],borderStyle),_drawLine([maxX+borderPadding,maxY],[0-borderPadding,maxY],borderStyle),_drawLine([0,maxY],[0,0],borderStyle)}function _drawLine(start,end,style){
var path=paper.path("M"+(_padding.left+start[0])+" "+(_padding.top+start[1])+"L"+(_padding.left+end[0])+" "+(_padding.top+end[1])).attr(style);return _addCssClass(path,"scene scene-grid"),path}function _drawTitle(text,style,x,y,angle){var textElement=paper.text(x,y,text).attr(style);angle&&textElement.rotate(angle,x,y)}function _addCssClass(raphaelObj,newClass){var pattern=new RegExp("\\b"+newClass+"\\b"),oldClass=raphaelObj.node.getAttribute("class")||"";raphaelObj.node.setAttribute("class",pattern.test(oldClass)?oldClass:oldClass+" "+newClass)}if("object"!=typeof rawOptions.x&&"object"!=typeof rawOptions.y)throw new Error("I need x and y axis")
;if(rawOptions.x.start>=rawOptions.x.end||rawOptions.y.start>=rawOptions.y.end)throw new Error("end must be greater than start");var clickableArea,options=_buildOptions(rawOptions),_x=options.x,_y=options.y,_xRange=Math.abs(_x.end-_x.start),_yRange=Math.abs(_y.end-_y.start),_width=_xRange*_x.unit,_height=_yRange*_y.unit,_xSubStepSize=_width/(_xRange/_x.step)/_x.subStep,_ySubStepSize=_height/(_yRange/_y.step)/_y.subStep,_color=options.color,_weight=options.weight,_axisTitlePositions=_getAxisTitlePositions(options),_padding=_getPadding(options,_axisTitlePositions),_axisTitleCoords=function(){var axisTitleCoords={abs:{},ord:{}}
;return axisTitleCoords.abs.x=_width/2,axisTitleCoords.abs.angle=0,"bottom"===_axisTitlePositions.abs?axisTitleCoords.abs.y=_height+options.axisTitlePadding:"top"===_axisTitlePositions.abs&&(axisTitleCoords.abs.y=-options.axisTitlePadding),axisTitleCoords.ord.y=_height/2,axisTitleCoords.ord.angle=-90,"left"===_axisTitlePositions.ord?axisTitleCoords.ord.x=-(options.axisTitlePadding+options.y.titleOffset):"right"===_axisTitlePositions.ord&&(axisTitleCoords.ord.x=_width+options.padding),axisTitleCoords}(),_labelCoords=function(){var labelCoords={abs:{},ord:{}};return labelCoords.abs.x=_width+options.borderWidth+options.labelPadding/2,labelCoords.abs.angle=0,
_y.start<0&&_y.end>0?labelCoords.abs.y=-1*_y.start*_y.unit:labelCoords.abs.y=_y.start>=0?0:_height,labelCoords.ord.y=-options.labelPadding,labelCoords.ord.angle=0,_x.start<0&&_x.end>0?labelCoords.ord.x=-1*_x.start*_x.unit:labelCoords.ord.x=_x.start>=0?0:_width,labelCoords}(),_snapToValues=function(){var snapValue,i,j,snapToValues={x:[],y:[]},xStepSize=_width/(_xRange/_x.step),yStepSize=_height/(_yRange/_y.step);for(i=_width/(_xRange/_x.firstStep),j=_xSubStepSize;j<=i;j+=_xSubStepSize)(snapValue=i-j)>=0&&snapToValues.x.splice(0,0,snapValue+_padding.left)
;for(Math.abs(_x.start)%_x.step&&snapToValues.x.splice(0,0,_padding.left);i<=_width;i+=xStepSize)for(j=0;j<xStepSize;j+=_xSubStepSize)(snapValue=i+j)<=_width&&snapToValues.x.push(snapValue+_padding.left);for(i=_height/(_yRange/_y.firstStep),j=_ySubStepSize;j<=i;j+=_ySubStepSize)(snapValue=i-j)>=0&&snapToValues.y.splice(0,0,snapValue+_padding.top);for(Math.abs(_y.start)%_y.step&&snapToValues.y.splice(0,0,_padding.top);i<=_height;i+=yStepSize)for(j=0;j<yStepSize;j+=_ySubStepSize)(snapValue=i+j)<=_height&&snapToValues.y.push(snapValue+_padding.top);return snapToValues}(),set=paper.set(),_borderBox={},arrowheadsShape="block-wide-long",obj={children:set,
snapping:options.snapping||!1,setColor:function(color){_color=String(color),set.remove().clear(),this.render()},setWeight:function(value){_weight=parseInt(value,10),set.remove().clear(),this.render()},getX:function(){return _borderBox.x},getY:function(){return _borderBox.y},getBBox:function(){return _.clone(_borderBox)},getWidth:function(){return _borderBox.width},getHeight:function(){return _borderBox.height},getUnits:function(){return{x:_x.unit,y:_y.unit}},getUnitSizes:function(){return{x:_borderBox.width/_x.unit,y:_borderBox.height/_y.unit}},getSubStepSizes:function(){return{x:_xSubStepSize,y:_ySubStepSize}},getCanvas:function(){return paper},
getOriginPosition:function(){return{left:_padding.left-_x.start*_x.unit,top:_padding.top-_y.start*_y.unit}},getPostionFromCartesian:function(x,y){var origin=this.getOriginPosition(),unitSizes=this.getUnits();return{left:origin.left+unitSizes.x*x,top:origin.top-unitSizes.y*y}},getGridBounds:function(){return{x:{start:_x.start,end:_x.end},y:{start:_y.start,end:_y.end}}},render:function(){_drawGrid(),_drawAxis(),_drawGraphTitle()},snap:function(x,y){return x=paper.raphael.snapTo(_snapToValues.x,x,_xSubStepSize/2),y=paper.raphael.snapTo(_snapToValues.y,y,_ySubStepSize/2),[x,y]},clickable:function(){clickableArea&&clickableArea.remove(),
clickableArea=paper.rect(_borderBox.x,_borderBox.y,_borderBox.width,_borderBox.height),clickableArea.attr({fill:"rgba(0,0,0,0)",stroke:0}),set.push(clickableArea)},toBack:function(shape){clickableArea&&shape.insertBefore(clickableArea)},toFront:function(shape){shape.toFront()}};return function(){var x=_padding.left,y=_padding.top;_borderBox={x:x,y:y,width:_width,height:_height,x2:x+_width,y2:y+_height}}(),obj.render(),obj}function _buildOptions(rawOptions){var options=_.merge({},{graphTitle:null,graphTitleRequired:!1,graphTitleSize:13,graphTitlePadding:40,color:"#222",weight:1,axisTitleSize:12,axisTitlePadding:36,scaleSize:9,labelSize:10,labelPadding:10,
padding:35,height:null,width:null,showOrigin:!0,showXAxisTitle:!0,showYAxisTitle:!0,showSubGrid:!1,subGridColor:"#aaaaaa",borderWidth:3,x:{start:-10,end:10,label:null,title:null,step:1,subStep:1,unit:10,color:"#222",weight:3,arrows:!0,showGrid:!0},y:{start:-10,end:10,label:null,title:null,step:1,subStep:1,unit:10,color:"#222",weight:3,arrows:!0,showGrid:!0,titleOffset:0}},rawOptions);if(options.x.firstStep=options.x.step,options.x.start<0){var d=options.x.start%options.x.step;0!=d&&(options.x.firstStep=Math.abs(d))}if(options.y.firstStep=options.y.step,options.y.start<0){var d=options.y.start%options.y.step;0!=d&&(options.y.firstStep=Math.abs(d))}
return options.width&&(options.x.unit=(options.width/Math.abs(options.x.end-options.x.start)).toPrecision(2)),options.height&&(options.y.unit=(options.height/Math.abs(options.y.end-options.y.start)).toPrecision(2)),options}function _getAxisTitlePositions(options){var axisTitlePositions={};return options.x.title&&(options.y.start<0?axisTitlePositions.abs="bottom":axisTitlePositions.abs="top"),options.y.title&&(options.x.start<0&&options.x.end<=0?axisTitlePositions.ord="right":axisTitlePositions.ord="left"),axisTitlePositions}function _getPadding(options,_axisTitlePositions){var padding={top:options.padding,right:options.padding,bottom:options.padding,
left:options.padding};return options.graphTitle&&!0===options.graphTitleRequired&&(padding.top+=options.graphTitlePadding),"top"===_axisTitlePositions.abs&&options.x.title?padding.top+=options.axisTitlePadding:options.y.label&&(padding.top+=options.labelPadding),"bottom"===_axisTitlePositions.abs&&options.x.title&&(padding.bottom+=options.axisTitlePadding),"right"===_axisTitlePositions.ord&&options.y.title?padding.right+=options.axisTitlePadding:options.x.label&&(padding.right+=options.labelPadding),"left"===_axisTitlePositions.ord&&options.y.title&&(padding.left+=options.axisTitlePadding),padding.left+=options.y.titleOffset,padding}
return gridFactory.getPaperSize=function(rawOptions){var options=_buildOptions(rawOptions),width=Math.abs(options.x.end-options.x.start)*options.x.unit,height=Math.abs(options.y.end-options.y.start)*options.y.unit,axisTitlePositions=_getAxisTitlePositions(options),padding=_getPadding(options,axisTitlePositions);return{width:padding.left+width+padding.right,height:padding.top+height+padding.bottom}},gridFactory}),define("nmcPci/portableLib/graphFunction",["taoQtiItem/portableLib/lodash"],function(_){"use strict";function _round(number,decimal){var m=Math.pow(10,parseInt(decimal));return Math.round(number*m)/m}function _isPoint(point){
return void 0!==point.x&&void 0!==point.y}function checkPairOfPoints(point1,point2){return _isPoint(point1)&&_isPoint(point2)&&point1.x!==point2.x}function plotCurvedEquation(canvas,equation,config,calc){function jump(){x+=config.precision,pathMove=!0}function getBoundedTop(top){return top<topMin?topMin:top>topMax?topMax:top}function appendPath(newPosition){prefix="L",pathMove&&(pathMove=!1,currentPosition?path+="M"+currentPosition.left+" "+getBoundedTop(currentPosition.top):prefix="M"),path+=prefix+newPosition.left+" "+getBoundedTop(newPosition.top)}
for(var y,currentPosition,newPosition,x=config.start,topMin=-100,topMax=canvas.height+100,path="",pathMove=!0,prefix="";x<config.end;)y=calc(equation,x),!1!==y?(newPosition={left:_round(x*config.unitSize.x+config.origin.left,3),top:_round(-y*config.unitSize.y+config.origin.top,3)},newPosition.top>canvas.height+100||newPosition.top<-100?(appendPath(newPosition),currentPosition=!1,jump()):(appendPath(newPosition),currentPosition=newPosition,x+=config.precision)):jump();return canvas.path(path)}var quadratic={get:function(vertex,point){if(checkPairOfPoints(vertex,point)){var a=(point.y-vertex.y)/Math.pow(point.x-vertex.x,2)
;return[a,-2*a*vertex.x,vertex.y+a*Math.pow(vertex.x,2)]}},plot:function(canvas,equation,config){return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1],c=equation[2];return a*Math.pow(x,2)+b*x+c})}},exponential={get:function(point1,point2){if(checkPairOfPoints(point1,point2)&&point1.y*point2.y>0){var b=Math.log(point2.y/point1.y)/(point2.x-point1.x);return[point1.y/Math.exp(b*point1.x),b]}},plot:function(canvas,equation,config){return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1];return a*Math.exp(b*x)})}},cosine={get:function(inflection,start){
if(checkPairOfPoints(start,inflection)){var d=inflection.y,a=start.y-inflection.y,b=Math.PI/(2*(inflection.x-start.x));return[a,b,-b*start.x,d]}},plot:function(canvas,equation,config){return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1],c=equation[2],d=equation[3];return a*Math.cos(b*x+c)+d})}},logarithmic={get:function(point1,point2){if(checkPairOfPoints(point1,point2)&&point1.y!==point2.y&&point2.x*point1.x>0){var a=(point2.y-point1.y)/Math.log(point2.x/point1.x);return[a,Math.exp(point1.y/a)/point1.x]}return!1},plot:function(canvas,equation,config){
return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1];return b*x>0&&a*Math.log(b*x)})}},tangent={get:function(start,inflection){if(checkPairOfPoints(start,inflection)){var d=start.y,b=Math.PI/(4*(inflection.x-start.x)),c=-b*start.x;return[(inflection.y-d)/Math.tan(b*inflection.x+c),b,c,d]}},plot:function(canvas,equation,config){return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1],c=equation[2],d=equation[3];return a*Math.tan(b*x+c)+d})}};return{checkPairOfPoints:checkPairOfPoints,linear:{get:function(point1,point2){if(checkPairOfPoints(point1,point2)){
var a=(point2.y-point1.y)/(point2.x-point1.x);return[a,point1.y-a*point1.x]}},plot:function(canvas,equation,config){function calcY(x){return a*x+b}function calcX(y){return(y-b)/a}function calcTranslateX(y){return _round(config.origin.left+calcX(y)*config.unitSize.x,3)}function calcTranslateY(x){return _round(config.origin.top-calcY(x)*config.unitSize.y,3)}function checkIsLineOutOfAxisX(position,start,end){var updatedPosition=position;return position.left<borderStartLeft&&(updatedPosition.left=borderStartLeft,updatedPosition.top=calcTranslateY(start)),position.left>borderEndLeft&&(updatedPosition.left=borderEndLeft,updatedPosition.top=calcTranslateY(end)),
position}function checkIsLineOutOfAxisY(position,start,end){var updatedPosition=position;return position.top<borderStartTop&&(updatedPosition.left=calcTranslateX(end),updatedPosition.top=borderStartTop),position.top>borderEndTop&&(updatedPosition.left=calcTranslateX(start),updatedPosition.top=borderEndTop),position}var a=equation[0],b=equation[1];if(config.segment)var startPosition={left:_round(config.start*config.unitSize.x+config.origin.left,3),top:calcTranslateY(config.start)},endPosition={left:_round(config.end*config.unitSize.x+config.origin.left,3),top:calcTranslateY(config.end)};else{
var xLength=Math.abs(config.point2.x-config.point1.x),yLength=Math.abs(config.point2.y-config.point1.y),borderStartLeft=_round(config.borderX1,3),borderStartTop=_round(config.borderY1,3),borderEndLeft=_round(config.borderX2,3),borderEndTop=_round(config.borderY2,3);if(yLength>xLength){var startPosition={left:calcTranslateX(config.end_y),top:borderStartTop};startPosition=checkIsLineOutOfAxisX(startPosition,config.start,config.end);var endPosition={left:calcTranslateX(config.start_y),top:borderEndTop};endPosition=checkIsLineOutOfAxisX(endPosition,config.start,config.end)}else{var startPosition={left:borderStartLeft,top:calcTranslateY(config.start)}
;startPosition=checkIsLineOutOfAxisY(startPosition,config.start_y,config.end_y);var endPosition={left:borderEndLeft,top:calcTranslateY(config.end)};endPosition=checkIsLineOutOfAxisY(endPosition,config.start_y,config.end_y)}}var path="M"+startPosition.left+" "+startPosition.top+"L"+endPosition.left+" "+endPosition.top;return canvas.path(path)}},absolute:{get:function(start,point2){if(checkPairOfPoints(start,point2))return[(point2.y-start.y)/Math.abs(point2.x-start.x),-start.x,start.y]},plot:function(canvas,equation,config){function calc(equation,x){var a=equation[0],b=equation[1],c=equation[2];return a*Math.abs(x+b)+c}var startPosition={
left:_round(config.start*config.unitSize.x+config.origin.left,3),top:_round(-calc(equation,config.start)*config.unitSize.y+config.origin.top,3)},middlePosition={left:_round(-equation[1]*config.unitSize.x+config.origin.left,3),top:_round(-calc(equation,-equation[1])*config.unitSize.y+config.origin.top,3)},endPosition={left:_round(config.end*config.unitSize.x+config.origin.left,3),top:_round(-calc(equation,config.end)*config.unitSize.y+config.origin.top,3)},path="M"+startPosition.left+" "+startPosition.top+"L"+middlePosition.left+" "+middlePosition.top+"L"+endPosition.left+" "+endPosition.top;return canvas.path(path)}},quadratic:quadratic,
exponential:exponential,logarithmic:logarithmic,cosine:cosine,tangent:tangent,vertical:{get:function(point){if(void 0!==point.x)return[point.x]},plot:function(canvas,equation,config){var multiplier;multiplier=!0===config.segment?-1:1;var x=equation[0]*config.unitSize.x+config.origin.left,yStart=multiplier*config.start_y*config.unitSize.y+config.origin.top,yEnd=multiplier*config.end_y*config.unitSize.y+config.origin.top,path="M"+x+" "+yStart+"L"+x+" "+yEnd;return canvas.path(path)}}}}),define("nmcPci/portableLib/poligonFactory",["taoQtiItem/portableLib/lodash","nmcPci/portableLib/graphFunction"],function(_,graphFunction){"use strict"
;function _applyStyle(path,config){path.attr({stroke:config.color?config.color:_defaults.color,"stroke-width":config.thickness?config.thickness:_defaults.thickness,opacity:.25,fill:config.color?config.color:_defaults.color})}function PoligonFactory(grid,config0){var _this=this,canvas=grid.getCanvas(),config=_.clone(config0||{});config=_.defaults(config,_defaults);var bounds=grid.getGridBounds();config.unitSize=grid.getUnits(),config.origin=grid.getOriginPosition(),config.start=bounds.x.start,config.end=bounds.x.end,config.start_y=bounds.y.start,config.end_y=bounds.y.end,_this.poligon=function(conf,points){conf=_.defaults(conf||{},config);try{
var path="M"+_.map(points,function(point){return point.getX()+","+point.getY()}).join("L"),poligon=canvas.path(path);return _applyStyle(poligon,conf),poligon}catch(e){console.log(e)}return!1}}var _defaults={start:-10,end:10,precision:.01,opacity:.25,color:"#bb1a2a",thickness:3};return PoligonFactory}),define("css!nmcGraphBarInteraction/runtime/css/nmcGraphBarInteraction",[],function(){}),
define("nmcGraphBarInteraction/runtime/nmcGraphBarInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/scale.raphael","nmcPci/portableLib/gridFactory","nmcPci/portableLib/poligonFactory","css!nmcGraphBarInteraction/runtime/css/nmcGraphBarInteraction"],function($,qtiCustomInteractionContext,_,event,scaleRaphael,gridFactory,PoligonFactory){var buildGridConfig=function(rawConfig){var getBoolean=function(value,defaultValue){return void 0===value?defaultValue:!0===value||"true"===value
},xCategories=rawConfig.xCategories?"object"==typeof rawConfig.xCategories?rawConfig.xCategories:JSON.parse(rawConfig.xCategories):[],xWeight=parseInt(rawConfig.xWeight,10),xBorderWeight=parseInt(rawConfig.xBorderWeight,10),yWeight=parseInt(rawConfig.yWeight,10),yBorderWeight=parseInt(rawConfig.yBorderWeight,10),wideBars=getBoolean(rawConfig.wideBars,!1),gridConfig={graphTitle:rawConfig.graphTitle,graphTitleRequired:getBoolean(rawConfig.graphTitleRequired,!1),weight:parseInt(rawConfig.weight,10),width:parseInt(rawConfig.width,10),height:parseInt(rawConfig.height,10),popupValueTitle:rawConfig.popupValueTitle,wideBars:wideBars,
showXAxisTitle:!!rawConfig.horizontalBars,showYAxisTitle:!rawConfig.horizontalBars,xLabelAlign:parseInt(rawConfig.xLabelAlign,10),showBarTick:getBoolean(rawConfig.showBarTick,!1),showSubGrid:getBoolean(rawConfig.showSubGrid,!1),color:"#222",subGridColor:rawConfig.subGridColor,labelSize:12,labelFont:"Verdana Regular",x:{start:0,end:Math.max(xCategories.length+(wideBars?0:1),1),label:rawConfig.xLabel,title:rawConfig.xTitle,step:parseInt(rawConfig.xStep,10),subStep:parseInt(rawConfig.xSubStep,10),weight:xWeight>0?xWeight:xBorderWeight,allowOuter:getBoolean(rawConfig.xAllowOuter,!0),categories:function(){var categories=[""]
;return categories=categories.concat(_.map(xCategories,"title")),categories.push(""),categories}(),arrows:getBoolean(rawConfig.xArrows,!1),showGrid:!1,titleOffset:rawConfig.labelBBsize||0},y:{start:-1*parseInt(rawConfig.yEnd,10),end:-1*parseInt(rawConfig.yStart,10),label:rawConfig.yLabel,title:rawConfig.yTitle,step:parseFloat(rawConfig.yStep,10),subStep:parseInt(rawConfig.ySubStep,10),weight:yWeight>0?yWeight:yBorderWeight,allowOuter:getBoolean(rawConfig.yAllowOuter,!0),arrows:getBoolean(rawConfig.yArrows,!1),showGrid:!0},plot:{color:rawConfig.plotColor,thickness:parseInt(rawConfig.plotThickness,10)},point:{color:rawConfig.pointColor,
glow:getBoolean(rawConfig.pointGlow,!0),radius:parseInt(rawConfig.pointRadius,10)}};if(rawConfig.horizontalBars){var temp=gridConfig.x;gridConfig.x=gridConfig.y,gridConfig.y=temp,temp=gridConfig.x.start,gridConfig.x.start=-1*gridConfig.x.end,gridConfig.x.end=-1*temp,temp=gridConfig.y.start,gridConfig.y.start=-1*gridConfig.y.end,gridConfig.y.end=-1*temp}return gridConfig.x.step<.1&&(gridConfig.x.step=.1),gridConfig.y.step<.1&&(gridConfig.y.step=.1),gridConfig.x.subStep<1&&(gridConfig.x.subStep=1),gridConfig.y.subStep<1&&(gridConfig.y.subStep=1),gridConfig.x.weight>0==!1&&(gridConfig.x.weight=3),gridConfig.y.weight>0==!1&&(gridConfig.y.weight=3),gridConfig
},createCanvas=function($container,config){var paperSize=gridFactory.getPaperSize(config);return scaleRaphael($(".shape-container",$container)[0],paperSize.width,paperSize.height)},nmcGraphBarInteraction={id:-1,inQtiCreator:function(){return _.isUndefined(this._inQtiCreator)&&this.$container&&(this._inQtiCreator=this.$container.hasClass("tao-qti-creator-context")),this._inQtiCreator},getTypeIdentifier:function(){return"nmcGraphBarInteraction"},initialize:function(id,dom,config){function initGrid($container,gridConfig){return paper=createCanvas($container,gridConfig),
_.isObject(gridConfig.x)&&_.isObject(gridConfig.y)&&gridConfig.x.start<gridConfig.x.end&&gridConfig.y.start<gridConfig.y.end&&(grid=gridFactory(paper,gridConfig),poligonFactory=new PoligonFactory(grid,gridConfig.plot)),grid}function dragStart(){if(self.config.horizontalBars){this.ox=grid.getX(),this.ow=this.attr("width");var idx=this.data("categoryId"),category=categories[idx];category&&bar_tooltip(!0,category,this.attr("x")+this.attr("width"),this.attr("y")),_.forEach(popups,function(popup){var x=popup.attr().x;if(x)popup.ox=x;else{var cx=popup.attr().cx;popup.ocx=cx}})}else{this.oy=this.attr("y"),this.oh=this.attr("height")
;var idx=this.data("categoryId"),category=categories[idx];category&&bar_tooltip(!0,category,this.attr("x")+this.attr("width")/2,this.attr("y"))}}function dragMove(dx,dy){if(self.config.horizontalBars){var newX=grid.snap(this.ox+dx+this.ow,0)[0],gridX=grid.getX();newX>=gridX&&newX<=gridX+grid.getWidth()&&(this.attr({x:grid.getX(),width:Math.max(newX-grid.getOriginPosition().left,1)}),popupValue&&(value=(newX-grid.getOriginPosition().left)/grid.getUnits().x,value=Math.trunc(100*value)/100,popupValue.attr({text:value||0})),_.forEach(popups,function(popup){if(popup.attr().x){var ox=popup.ox;popup.attr({x:ox+dx})}else{var ocx=popup.ocx;popup.attr({cx:ocx+dx})}}))
}else{var newY=grid.snap(0,this.oy+dy)[1],gridY=grid.getY();if(newY>=gridY&&newY<=gridY+grid.getHeight()){var y=this.attr().y,d=newY-y;this.attr({y:newY,height:Math.max(grid.getOriginPosition().top-newY-1,1)}),popupValue&&(value=-(newY-grid.getOriginPosition().top)/grid.getUnits().y,value=Math.trunc(100*value)/100,popupValue.attr({text:value||0})),_.forEach(popups,function(popup){var y=popup.attr().y;if(y)popup.attr({y:y+d});else{var cy=popup.attr().cy;popup.attr({cy:cy+d})}})}}}function endDrag(){if(self.config.horizontalBars){var width=this.attr().width<=1?0:this.attr().width,value=width/grid.getUnits().x;setBarValue(this,value),
self.trigger("responseChange",[self.getResponse()]),bar_tooltip()}else{var y=this.attr().y,snap=grid.snap(0,y),value=-(snap[1]-grid.getOriginPosition().top)/grid.getUnits().y;setBarValue(this,value),self.trigger("responseChange",[self.getResponse()]),bar_tooltip()}}function bar_tooltip(show,category,x,y){if(!show)return _.forEach(popups,function(popup){popup.remove()}),popups=[],void(popupValue=null);var line1=paper.text(x+5,y,category.title).attr({"text-anchor":"start"}),bb1=line1.getBBox(),circle=paper.circle(bb1.x+5,bb1.y2+10,5).attr({fill:category.color,stroke:category.color
}),title=paper.text(bb1.x+20,y+bb1.height+5,(self.config.popupValueTitle||"")+" :").attr({"text-anchor":"start"}),bb2=title.getBBox();popupValue=paper.text(bb2.x2+5,y+bb1.height+5,category.value||"0").attr({"text-anchor":"start","font-weight":"bold"});var bb3=popupValue.getBBox(),rect=paper.rect(bb1.x-5,bb1.y-5,Math.max(bb1.width,bb3.x2-bb1.x)+30,bb3.y2-bb1.y+10,4).attr({fill:"white",stroke:category.color});if(popups.push(line1),popups.push(circle),popups.push(title),popups.push(popupValue),popups.push(rect),self.config.width-(rect.attr().x+rect.attr().width)<0){var w=rect.attr().width;_.forEach(popups,function(popup){var px=popup.attr().x;if(px)popup.attr({
x:px-w});else{var cx=popup.attr().cx;popup.attr({cx:cx-w})}})}circle.toFront(),title.toFront(),line1.toFront(),popupValue.toFront()}function drawBars(categories){if(self.config.horizontalBars)return void drawYBars(categories);var labelY=grid.getOriginPosition().top,tickColor=self.gridConfig.color,tickAttr={stroke:tickColor};tickAttr["stroke-width"]=1,bars=[];var barX=grid.getX()+1;categories.forEach(function(category,index){var y=grid.getHeight()+grid.getY()-4,xUnits=grid.getUnits().x,x=self.gridConfig.wideBars?barX:xUnits*(index+1)-xUnits/4+grid.getX(),barWidth=xUnits/(self.gridConfig.wideBars?1:2),rect=paper.rect(x,y,barWidth,3);rect.attr({
"stroke-width":0,fill:category.color}),barX+=barWidth,rect.data("categoryId",index),!self.inQtiCreator()&&category.isStatic||(rect.attr({cursor:"ns-resize"}),rect.drag(dragMove,dragStart,endDrag)),bars.push(rect),category.isStatic&&setBarValue(rect,category.value);var anchor="middle",labelX=x+barWidth/2;switch(self.gridConfig.xLabelAlign){case 0:anchor="start",labelX=x;break;case 2:anchor="end",labelX=x+barWidth}paper.text(labelX,labelY+16,category.title).attr({font:"12pt Verdana Regular","text-anchor":anchor}),self.gridConfig.showBarTick&&paper.path("M"+labelX+" "+(labelY+1.5)+"L"+labelX+" "+(labelY+10)).attr(tickAttr)})}function drawYBars(categories){
var labelX=grid.getOriginPosition().left,tickColor=self.gridConfig.color,tickAttr={stroke:tickColor},maxLabelWidth=0,barY=grid.getY()+1;tickAttr["stroke-width"]=1,bars=[],categories.forEach(function(category,index){var labelY,barText,labelYAligmentOffset,aligmentOffset,x=grid.getX(),yUnits=grid.getUnits().y,y=self.gridConfig.wideBars?barY:yUnits*(index+1)-yUnits/4+grid.getY(),barWidth=yUnits/(self.gridConfig.wideBars?1:2),rect=paper.rect(x,y,3,barWidth);rect.attr({"stroke-width":0,fill:category.color}),barY+=barWidth,rect.data("categoryId",index),!self.inQtiCreator()&&category.isStatic||(rect.attr({cursor:"ew-resize"}),rect.drag(dragMove,dragStart,endDrag)),
bars.push(rect),category.isStatic&&setBarValue(rect,category.value),self.config.horizontalBarLabels?(labelY=y+barWidth/2,barText=paper.text(labelX-9,labelY,category.title).attr({font:"12pt Verdana Regular","text-anchor":"end"})):(labelYAligmentOffset=1-self.gridConfig.xLabelAlign/2,labelY=y+labelYAligmentOffset*barWidth,barText=paper.text(labelX-20,labelY,category.title).attr({font:"12pt Verdana Regular"}),barText.rotate(270),aligmentOffset=.5-self.gridConfig.xLabelAlign/2,barText.translate(aligmentOffset*barText.getBBox().height,0)),self.gridConfig.showBarTick&&paper.path("M"+(labelX-8.5)+" "+labelY+"L"+labelX+" "+labelY).attr(tickAttr),
maxLabelWidth=Math.max(barText.getBBox().width,maxLabelWidth)});var interaction=self._taoCustomInteraction;Math.abs(interaction.prop("labelBBsize")-maxLabelWidth)>1&&(interaction.prop("labelBBsize",maxLabelWidth),interaction.triggerPci("configChange",[interaction.getProperties()]))}function setBarValue(bar,value){if(value=value||0,self.config.horizontalBars){var xUnits=grid.getUnits().x,width=xUnits*value;bar.attr({x:grid.getX(),width:Math.max(width,3)})}else{var yUnits=grid.getUnits().y,height=yUnits*value;bar.attr({y:grid.getHeight()+grid.getY()-Math.max(height,4),height:Math.max(height,3)})}var idx=bar.data("categoryId"),category=categories[idx]
;category&&(category.value=value||"0")}function getCategories(rawConfig){return rawConfig.xCategories?"object"==typeof rawConfig.xCategories?_.cloneDeep(rawConfig.xCategories):JSON.parse(rawConfig.xCategories):[]}event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{};var paper,grid,popupValue,self=this,$container=$(dom),categories=[],bars=[],popups=[];this.gridConfig=buildGridConfig(this.config),grid=initGrid($container,this.gridConfig),categories=getCategories(config),drawBars(categories),self.on("configChange",function(newRawConfig){self.config=newRawConfig,self.gridConfig=buildGridConfig(newRawConfig),initGrid($container,self.gridConfig),
categories=getCategories(newRawConfig),drawBars(categories)}),this.getRawResponse=function(){return categories},this.setRawResponse=function(response){response&&response.list&&response.list.float&&response.list.float.forEach(function(value,idx){var bar=bars[idx];bar&&setBarValue(bar,value)})}},setResponse:function(response){this.setRawResponse(response)},getResponse:function(){var categories=this.getRawResponse();return{list:{float:_.map(categories,function(category){return category.value||0})}}},resetResponse:function(){},destroy:function(){},setSerializedState:function(state){},getSerializedState:function(){return{response:this.getResponse()}},
resetSerializedState:function(){},getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()}};qtiCustomInteractionContext.register(nmcGraphBarInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))
}(".nmcGraphBarInteraction .tools-container {margin-left: 30px;}\r\n.nmcGraphBarInteraction .tools-container .btn-reset {display: none;}"),define(["nmcGraphBarInteraction/runtime/nmcGraphBarInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=nmcGraphBarInteraction.min.js.map