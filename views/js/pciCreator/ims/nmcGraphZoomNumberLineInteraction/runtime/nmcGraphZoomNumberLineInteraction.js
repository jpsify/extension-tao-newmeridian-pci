...
define([
    ...
    'nmcPci/portableLib/axisFactory',
    'nmcGraphZoomNumberLineInteraction/runtime/libs/axisZoom',
    'css!nmcGraphZoomNumberLineInteraction/runtime/css/nmcGraphZoomNumberLineInteraction'
], function($, qtiCustomInteractionContext, _, event, scaleRaphael, pointFactory, axisFactory, axisZoom){

    'use strict';

    var _typeIdentifier = 'nmcGraphZoomNumberLineInteraction';

    var nmcGraphZoomNumberLineInteraction = {
        /**
        ...
        initialize : function(id, dom, config){
            ...
            function findRect(allRects, position){
                ...
                _.each(allRects, function(rect){
                    ...
                    if(pos <= position && position <= rect.position + stepWidth){
                        ret = rect;
                        return false;
                    }
                });
            ...
            function zoomOnRect(x) {
                ...
                if(rect){
                    ...
                    zoomAxis.setConfig('labels', [_format(selectedRect.coord), _format((selectedRect.coord + subDivisionIncrement * _this.config.increment))]);
                    zoomAxis.render();
            ...
            function getZoomPointCoordinate(){
                return selectedRect.coord + zoomPoint.getCartesianCoord().x / _this.axisConfig.unitSubDivision * _this.config.increment;
            }
            ...
            function getSelectedPointPositionLeft(){
                if(selectedCoord && selectedRect){
                    var zoomAxisConfig = zoomAxis.getConfig();
                    var left = (selectedCoord - selectedRect.coord) * zoomAxisConfig.unitSize * _this.axisConfig.unitSubDivision / _this.axisConfig.increment + zoomAxisConfig.left;
            ...
            function updateAxisPoint(){
                ...
                //draw to axis point
                //TODO: drawAxisPoint(selectedCoord); 
            ...
            function initAxis($canvasContainer, axisConfig){
                ...
                axisSizePx = (axisConfig.max - axisConfig.min) / axisConfig.increment * axisConfig.unitSize;
                zoomAxisConfig = _.defaults({
                    min : 0,
                    max : 1,
                    top : axisConfig.top + 150,
                    unitSize : axisSizePx,
                    unitSubDivision : axisConfig.zoomSubDivision,
                    arrows : false,
                    increment: 1,
                    labels : ['A', 'B'],
                }, axisConfig);
            ...
            this.setRawResponse = function(selected){
                ...
                axisSelectedX = axis.coordinateToPosition(selected).left;
                if (axisSelectedX) {
    ...
    function createCanvas($container, config){
        ...
        var padding = 30;
        var width = config.left + 2 * padding + config.unitSize * (config.max - config.min) / config.increment;
    ...
    function buildAxisConfig(rawConfig){
        ...
        var config = _.merge(_default, {
            ...
            zoomSubDivision : (rawConfig.zoomSubDivision === undefined) ? undefined : parseInt(rawConfig.zoomSubDivision),
            increment: (rawConfig.increment === undefined) ? undefined : parseInt(rawConfig.increment),
            width: (rawConfig.width === undefined) ? undefined : parseInt(rawConfig.width),
            point : {
                color : rawConfig.graphColor
            },
        });
        config.unitSize = config.width / (config.max - config.min) * config.increment;
        return config;
    }
    ...
    qtiCustomInteractionContext.register(nmcGraphZoomNumberLineInteraction);
});
