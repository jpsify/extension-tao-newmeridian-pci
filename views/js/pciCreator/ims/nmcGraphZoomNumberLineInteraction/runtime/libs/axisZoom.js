...
define(['taoQtiItem/portableLib/lodash'], function(_){
    ...
    function createZoomable(axis){
        ...
        var paper = axis.getCanvas(),
            ...
            subDivisionIncrement = 1 / config.unitSubDivision * config.increment,
            left = config.left,
            top = config.top - config.divisionWidth;

        function addRectangle(position, coord, width){

            var rect = paper.rect(position, top, width, config.divisionWidth * 2);
        ...
        for(var i = config.min; i < config.max; i += config.increment){
            ...
            if(i < config.max){
                ...
                var left_ = left, i_ = i;
                for(var j = 1; j <= config.unitSubDivision && i_ < config.max; j++){
                    //build rectangle :
                    addRectangle(left_, i_, subDivisionSize);
