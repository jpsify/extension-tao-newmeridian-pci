...
center&&(wrapper.style.position="absolute",wrapper.style.left=parseInt((w-newWidth)/2)+"px",wrapper.style.top=parseInt((h-newHeight)/2)+"px")},paper.scaleAll=function(amount){paper.changeSize(width*amount,height*amount)},paper.changeSize(width,height),paper.w=width,paper.h=height,paper}}),define("nmcPci/portableLib/pointFactory",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash"],function($,_){"use strict";function pointFactory(paper,grid,options){options=_.defaults(options||{},_defaults)
...
define("nmcPci/portableLib/axisFactory",["taoQtiItem/portableLib/lodash"],function(_){"use strict";function getArrowsPath(orientation,config){var tipPosition,arrowConfig={length:1.8*config.divisionWidth,tip:config.divisionWidth,orientation:orientation},increment=config.increment||_defaults.increment,axisSizePx=(config.max-config.min)/increment*config.unitSize,path="";if("right"===arrowConfig.orientation)tipPosition=config.left+axisSizePx+arrowConfig.length,path+="M"+(config.left+axisSizePx)+","+config.top,path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition-arrowConfig.tip)+","+(config.top+.9*config.divisionWidth),path+="L"+tipPosition+","+config.top,
path+="M"+(tipPosition-arrowConfig.tip)+","+(config.top-.9*config.divisionWidth),path+="L"+tipPosition+","+config.top;else{if("left"!==arrowConfig.orientation)throw"unknown orientation type to the arrow";tipPosition=config.left-arrowConfig.length,path+="M"+config.left+","+config.top,path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition+arrowConfig.tip)+","+(config.top+.9*config.divisionWidth),path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition+arrowConfig.tip)+","+(config.top-.9*config.divisionWidth),path+="L"+tipPosition+","+config.top}return path}function _applyStyle(path,config){path.attr({stroke:config.color?config.color:_defaults.color,
"stroke-width":config.thickness?config.thickness:_defaults.thickness,opacity:config.opacity?config.opacity:_defaults.opacity})}function addCssClass(raphaelObj,newClass){var pattern=new RegExp("\\b"+newClass+"\\b"),oldClass=raphaelObj.node.getAttribute("class")||"";raphaelObj.node.setAttribute("class",pattern.test(oldClass)?oldClass:oldClass+" "+newClass)}function getLabel(i,config){return config&&_.isArray(config.labels)&&void 0!==config.labels[i]&&!1!==config.labels[i]?config.labels[i]:i}function axisFactory(paper,config){config=_.defaults(config||{},_defaults);var arrowRight,arrowLeft,axisPath,set=paper.set(),steps=[],obj={getMin:function(){
return config.min},getMax:function(){return config.max},getCanvas:function(){return paper},getOriginPosition:function(){return{left:config.left-config.unitSize*config.min/config.increment,top:config.top}},snap:function(x0){var step=config.unitSize/config.unitSubDivision;return[paper.raphael.snapTo(steps,x0,step/2),config.top]},render:function(){this.clear();var path="M"+config.left+","+config.top,axisSizePx=(config.max-config.min)/config.increment*config.unitSize,subDivisionSize=config.unitSize/config.unitSubDivision,subStep=config.increment/config.unitSubDivision;path+="L"+(config.left+axisSizePx)+","+config.top;var position=config.left;steps.push(position)
;for(var i=config.min;i<=config.max;i+=config.increment){path+="M"+position+","+(config.top-config.divisionWidth),path+="L"+position+","+(config.top+config.divisionWidth);var label=paper.text(position,config.top-config.divisionWidth-config.fontSize/2-5,getLabel(i,config));if(label.attr({"font-size":config.fontSize}),addCssClass(label,"scene scene-text"),set.push(label),i<config.max){var subPosition=position+subDivisionSize;steps.push(subPosition);for(var j=config.unitSubDivision,p=i+subStep;j>1&&p<=config.max;j--,p+=subStep){if(p==config.max){path+="M"+subPosition+","+(config.top-config.divisionWidth),
path+="L"+subPosition+","+(config.top+config.divisionWidth);var label=paper.text(subPosition,config.top-config.divisionWidth-config.fontSize/2-5,getLabel(config.max,config));label.attr({"font-size":config.fontSize}),addCssClass(label,"scene scene-text"),set.push(label)}else path+="M"+subPosition+","+(config.top-config.subDivisionWidth),path+="L"+subPosition+","+(config.top+config.subDivisionWidth);subPosition+=subDivisionSize,j!==config.unitSubDivision&&steps.push(subPosition)}}position+=config.unitSize,steps.push(position)}config.arrows&&(arrowRight=this.buildArrow("right"),arrowLeft=this.buildArrow("left")),axisPath=paper.path(path),
addCssClass(axisPath,"scene scene-grid"),_applyStyle(axisPath,config),set.push(axisPath)},getSet:function(){return set},isRendered:function(){return set.length},clear:function(){set.length>0&&(steps=[],set.remove().clear())},clickable:function(){var borderBox=axisPath.getBBox(),clickableArea=paper.rect(borderBox.x,borderBox.y,borderBox.width,borderBox.height);return clickableArea.attr({fill:"rgba(0,0,0,0)",stroke:0}),set.push(clickableArea),clickableArea},getWidth:function(){return set.getBBox().width},getHeight:function(){return set.getBBox().height},getUnits:function(){return{x:config.unitSize,y:0}},coordinateToPosition:function(x){return{
left:this.getOriginPosition().left+this.getUnits().x*x/this.getConfig().increment,top:this.getOriginPosition().top}},buildArrow:function(orientation){var pathStr=getArrowsPath(orientation,config),arrow=paper.path(pathStr);return addCssClass(arrow,"scene scene-grid"),_applyStyle(arrow,config),set.push(arrow),arrow},getConfig:function(){return _.clone(config)},setConfig:function(key,value){config[key]=value},buildContainerBox:function(options){var _defaults={padding:20,absolute:!0,shadow:!1};options=_.defaults(options||{},_defaults);var box,padding=options.padding,bb=set.getBBox()
;if(options.absolute)box=paper.rect(bb.x-padding,bb.y-padding,bb.width+2*padding,bb.height+2*padding);else{var axisSizePx=(config.max-config.min)*config.unitSize;box=paper.rect(config.left-padding,config.top-padding,axisSizePx+2*padding,config.divisionWidth+2*padding)}return box.attr({fill:"#fff"}),set.push(box),box.insertBefore(set),options&&options.shadow&&set.push(box.glow({width:5,fill:!0,offsetx:3,offsety:3,color:"#999"})),box},addCssClass:addCssClass};return obj.render(),obj}var _defaults={top:40,left:40,min:-5,max:5,labels:!1,thickness:2,color:"#000",divisionWidth:16,subDivisionWidth:8,unitSubDivision:2,unitSize:50,fontSize:18,arrows:!1,opacity:1,
increment:1};return axisFactory}),define("nmcGraphZoomNumberLineInteraction/runtime/libs/axisZoom",["taoQtiItem/portableLib/lodash"],function(_){"use strict";function createZoomable(axis){for(var clickables=[],paper=axis.getCanvas(),config=axis.getConfig(),clickableSet=paper.set(),subDivisionSize=config.unitSize/config.unitSubDivision,subDivisionIncrement=1/config.unitSubDivision*config.increment,left=config.left,top=config.top-config.divisionWidth,i=config.min;i<config.max;i+=config.increment){if(i<config.max)for(var left_=left,i_=i,j=1;j<=config.unitSubDivision&&i_<config.max;j++)!function(position,coord,width){
var rect=paper.rect(position,top,width,2*config.divisionWidth);rect.attr({stroke:"#1a6596",fill:"#e6eef4"}),rect.hide(),clickableSet.push(rect),clickables.push({position:position,rect:rect,coord:coord,active:!1})}(left_,i_,subDivisionSize),i_+=subDivisionIncrement,left_+=subDivisionSize;left+=config.unitSize}return clickableSet.insertBefore(axis.getSet()),clickables}function drawPath(paper,points){var pathStr="";return _.each(points,function(point){pathStr+=pathStr?"L":"M",pathStr+=point.toString()}),pathStr+="Z",paper.path(pathStr).attr({stroke:"#1a6596",fill:"#e6eef4"})}function drawZoomEffect(paper,from,to){
var source=from.getBBox(),target=to.getBBox(),points=[[source.x,source.y+source.height],[source.x+source.width,source.y+source.height],[target.x+target.width,target.y],[target.x,target.y]],zoomEffect=drawPath(paper,points);return zoomEffect&&(to.zoomEffect&&to.zoomEffect.remove(),to.zoomEffect=zoomEffect,zoomEffect.insertBefore(to)),zoomEffect}return{createZoomable:createZoomable,drawZoomEffect:drawZoomEffect}}),define("css!nmcGraphZoomNumberLineInteraction/runtime/css/nmcGraphZoomNumberLineInteraction",[],function(){}),
define("nmcGraphZoomNumberLineInteraction/runtime/nmcGraphZoomNumberLineInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/scale.raphael","nmcPci/portableLib/pointFactory","nmcPci/portableLib/axisFactory","nmcGraphZoomNumberLineInteraction/runtime/libs/axisZoom","css!nmcGraphZoomNumberLineInteraction/runtime/css/nmcGraphZoomNumberLineInteraction"],function($,qtiCustomInteractionContext,_,event,scaleRaphael,pointFactory,axisFactory,axisZoom){"use strict";function createCanvas($container,config){
var width=config.left+60+config.unitSize*(config.max-config.min)/config.increment;return scaleRaphael($(".shape-container",$container)[0],width,260)}function buildAxisConfig(rawConfig){var _default={top:60,left:50,unitSize:50,min:0,max:10,unitSubDivision:2,zoomSubDivision:10,arrows:!0,increment:1,width:740,point:{color:"#266d9c"}},config=_.merge(_default,{min:void 0===rawConfig.min?void 0:parseInt(rawConfig.min),max:void 0===rawConfig.max?void 0:parseInt(rawConfig.max),unitSubDivision:void 0===rawConfig.unitSubDivision?void 0:parseInt(rawConfig.unitSubDivision),zoomSubDivision:void 0===rawConfig.zoomSubDivision?void 0:parseInt(rawConfig.zoomSubDivision),
increment:void 0===rawConfig.increment?void 0:parseInt(rawConfig.increment),width:void 0===rawConfig.width?void 0:parseInt(rawConfig.width),point:{color:rawConfig.graphColor}});return config.unitSize=config.width/(config.max-config.min)*config.increment,config}function _format(num){var str=num+"";return str.length>5&&(str.match(/99999\d$/)?str=Math.round(1e4*num)/1e4:(str=Math.floor(1e3*num)/1e3,str=str.toString(),str.match(/^\d+\.\d{3,}$/)&&(str+="..."))),str}var nmcGraphZoomNumberLineInteraction={getInstance:function(dom,config,state){var response=config.boundTo
;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties,config.assetManager),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var value=this.getRawResponse();return value?{base:{float:value}}:{base:null}},destroy:function(){$(this.dom).off().empty()},getTypeIdentifier:function(){return"nmcGraphZoomNumberLineInteraction"},initialize:function(id,dom,config){function findRect(allRects,position){
var ret,positions=_.pluck(allRects,"position"),stepWidth=positions[1]-positions[0];return position=parseInt(position),_.each(allRects,function(rect){if(parseInt(rect.position)<=position&&position<=rect.position+stepWidth)return ret=rect,!1}),ret}function zoomOnRect(x){var rect=findRect(rects,x);if(rect){selectedRect&&selectedRect.rect.hide(),zoomEffect&&zoomEffect.remove(),selectedRect=rect,zoomAxis.addCssClass(selectedRect.rect,"scene scene-selected-rectangle"),selectedRect.rect.show(),zoomAxis.setConfig("labels",[_format(selectedRect.coord),_format(selectedRect.coord+subDivisionIncrement*_this.config.increment)]),zoomAxis.render()
;var containerBox=zoomAxis.buildContainerBox({shadow:!0});zoomAxis.addCssClass(containerBox,"scene scene-zoom-popup");var left=getSelectedPointPositionLeft();void 0!==left&&drawZoomAxisPoint(left),zoomAxis.clickable().click(function(e){drawZoomAxisPoint(e.layerX),updateAxisPoint()}),zoomEffect=axisZoom.drawZoomEffect(paper,selectedRect.rect,containerBox),zoomAxis.addCssClass(zoomEffect,"scene scene-zoom-effect")}}function drawZoomAxisPoint(left){zoomPoint&&zoomPoint.remove();var zoomAxisConfig=zoomAxis.getConfig(),pointConfig={x:left,axis:"x",xMin:zoomAxisConfig.left,xMax:zoomAxisConfig.left+zoomAxisConfig.unitSize,removable:!1,on:{dragStop:updateAxisPoint}}
;return pointConfig=_.defaults(pointConfig,_this.axisConfig.point),zoomPoint=pointFactory(paper,zoomAxis,pointConfig),zoomPoint.render(),zoomPoint.drag(),zoomAxis.getSet().push(zoomPoint.children),zoomPoint}function getZoomPointCoordinate(){return selectedRect.coord+zoomPoint.getCartesianCoord().x/_this.axisConfig.unitSubDivision*_this.config.increment}function getSelectedPointPositionLeft(){if(selectedCoord&&selectedRect){var zoomAxisConfig=zoomAxis.getConfig(),left=(selectedCoord-selectedRect.coord)*zoomAxisConfig.unitSize*_this.axisConfig.unitSubDivision/_this.axisConfig.increment+zoomAxisConfig.left;if(left=parseInt(_format(left)),
zoomAxisConfig.left<=left&&left<=zoomAxisConfig.left+zoomAxisConfig.unitSize)return left}}function updateAxisPoint(){selectedCoord=getZoomPointCoordinate(),_.defer(function(){_this.trigger("responseChange",[_this.getResponse()])})}function initAxis($canvasContainer,axisConfig){var axisSizePx,zoomAxisConfig;subDivisionIncrement=1/axisConfig.unitSubDivision,paper=createCanvas($canvasContainer,axisConfig),axis=new axisFactory(paper,axisConfig),axisSizePx=(axisConfig.max-axisConfig.min)/axisConfig.increment*axisConfig.unitSize,zoomAxisConfig=_.defaults({min:0,max:1,top:axisConfig.top+150,unitSize:axisSizePx,unitSubDivision:axisConfig.zoomSubDivision,arrows:!1,
increment:1,labels:["A","B"]},axisConfig),zoomAxis=new axisFactory(paper,zoomAxisConfig),zoomAxis.getSet().hide(),rects=axisZoom.createZoomable(axis),axis.clickable().click(function(e){zoomOnRect(e.layerX)})}function reset(){selectedRect&&(selectedRect.rect.hide(),selectedRect=void 0),zoomEffect&&(zoomEffect.remove(),zoomEffect=void 0),zoomAxis&&zoomAxis.getSet().remove().clear(),axisPoint&&(axisPoint.remove(),axisPoint=void 0),selectedCoord=void 0}function setAxis(axisConfig){_this.axisConfig=buildAxisConfig(_.merge(_this.config,_this.axisConfig,axisConfig)),initAxis($container,_this.axisConfig),reset()}event.addEventMgr(this),this.id=id,this.dom=dom,
this.config=config||{};var paper,axis,zoomAxis,zoomEffect,selectedRect,selectedCoord,subDivisionIncrement,axisPoint,zoomPoint,rects,$container=$(dom),_this=this;this.reset=function(){reset()},this.getRawResponse=function(){return selectedCoord},this.setRawResponse=function(selected){var axisSelectedX,zoomAxisSelectedX;selectedCoord=selected,(axisSelectedX=axis.coordinateToPosition(selected).left)&&(zoomOnRect(axisSelectedX),selectedRect&&(zoomAxisSelectedX=getSelectedPointPositionLeft(),drawZoomAxisPoint(zoomAxisSelectedX),updateAxisPoint()))},this.axisConfig=buildAxisConfig(this.config),initAxis($container,this.axisConfig),this.on("axischange",setAxis)},
setResponse:function(response){response&&response.base&&response.base.float&&this.setRawResponse(response.base.float)},resetResponse:function(){this.reset()},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){return this.getResponse()}};qtiCustomInteractionContext.register(nmcGraphZoomNumberLineInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))
}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\r\n\r\n.nmcGraphZoomNumberLineInteraction {\r\n    width:100%;\r\n    padding:30px 30px 50px;\r\n    margin:20px 0;\r\n    padding:20px;\r\n    border : 6px solid #f5f3f2;\r\n}\r\n\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .clearfix {\r\n    overflow: auto;\r\n}\r\n\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-available {float:left; position : relative; border : 6px solid #f5f3f2; padding:4px; margin-right : 10px; margin-bottom:10px;}\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-available .intervals-overlay {position:absolute; width:100%; height : 100%; top:0; left:0; background: #ddd; opacity : 0.6; display:none;}\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-available .interval-available {display:none;}\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-available .interval-available.activated {display:inline-block;}\r\n\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-selected {float:left;}\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-selected .interval {float:left; position : relative; width:92px; border : 6px solid #f5f3f2; padding:4px; margin-right:10px;}\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-selected .interval .deleter {cursor: pointer; }\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-selected .interval .deleter:before {font-size:1.25rem; margin-right: 3px;}\r\n\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-selected .interval .btn {margin-bottom:10px;}\r\n\r\n.nmcGraphZoomNumberLineInteraction .shape-controls button {padding: 0 6px 8px; height:36px; width:72px; margin-right:2px;}\r\n.nmcGraphZoomNumberLineInteraction .shape-controls button img {height:20px;width:50px;}\r\n\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-template {display:none;}\r\n\r\n.nmcGraphZoomNumberLineInteraction .shape-controls .intervals-row {margin-bottom: 2px;}\r\n#creator-graphFunctionInteraction-axis input[data-increment] { width: 60px!important}\r\n"),
define(["nmcGraphZoomNumberLineInteraction/runtime/nmcGraphZoomNumberLineInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=nmcGraphZoomNumberLineInteraction.min.js.map