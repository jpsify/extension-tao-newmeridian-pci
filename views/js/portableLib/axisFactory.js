...
define(['taoQtiItem/portableLib/lodash'], function(_){
    ...
    function getArrowsPath(orientation, config){
        ...
        var arrowConfig = {
            length : config.divisionWidth * 1.8,
            tip : config.divisionWidth,
            orientation : orientation
        };

        var increment = config.increment || _defaults.increment;

        var axisSizePx = (config.max - config.min) / increment * config.unitSize;
        var tipPosition, path = '', a = 0.9;
        ...
        if(arrowConfig.orientation === 'right'){
            ...
            path += 'M' + (tipPosition - arrowConfig.tip) + ',' + (config.top + config.divisionWidth * a);
            path += 'L' + tipPosition + ',' + config.top;
    ...
    function axisFactory(paper, config){
        ...
        var obj = {
            ...
            getOriginPosition : function(){
                return {
                    left : config.left - config.unitSize * config.min / config.increment,
                    top : config.top
                };
            },
            snap : function(x0){
            ...
            render : function(){
                ...
                var axisSizePx = (config.max - config.min) / config.increment * config.unitSize;
                var subDivisionSize = config.unitSize / config.unitSubDivision;
                var subStep = config.increment / config.unitSubDivision;
                ...
                steps.push(position);
                for(var i = config.min; i <= config.max; i += config.increment){
                    ...
                    if(i < config.max){
                        ...
                        for(var j = config.unitSubDivision, p = i + subStep; j > 1 && p <= config.max; j--, p += subStep){
                            if (p == config.max) {
                                path += 'M' + subPosition + ',' + (config.top - config.divisionWidth);
                                path += 'L' + subPosition + ',' + (config.top + config.divisionWidth);
                                    var label = paper.text(subPosition, config.top - config.divisionWidth - config.fontSize / 2 - 5, getLabel(config.max, config));
                                label.attr({
                                    'font-size' : config.fontSize
                                });
                                addCssClass(label, 'scene scene-text');
                ...
                axisPath = paper.path(path);
                addCssClass(axisPath, 'scene scene-grid');
                _applyStyle(axisPath, config);
                set.push(axisPath);
            },
            getSet : function(){
                return set;
            ...
            clickable : function(){
                ...
                var borderBox = axisPath.getBBox();// set.getBBox();
            ...
            coordinateToPosition : function(x){
                return {
                    left : this.getOriginPosition().left + this.getUnits().x * x / this.getConfig().increment,
